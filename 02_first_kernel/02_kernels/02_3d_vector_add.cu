...
__global__ void vector_add_gpu_3d(float *a, float *b, float *c, int nx, int ny, int nz) 
{
    ...
    if (i < nx && j < ny && k < nz) 
    {
        ...
        if (idx < nx * ny * nz) c[idx] = a[idx] + b[idx];
    }
    // you get the point...
}
...
int main()
{
    ...
    // we want to handle all the elements possible, so we effectively add one more block to handle all the elements.
    ...
    double gpu_1d_avg_time = gpu_1d_total_time / 5.0;
